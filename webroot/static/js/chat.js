...
function showOnlineList(dataObj) {
    ...
    let sub=function(str,n){
        var r=/[^\x00-\xff]/g;
        let count = str.replace(r,"mm").length;
    ...
    for (var i = 0; i < dataObj.list.length; i++) {
        if (dataObj.list[i].avatar) {
            img = "<li id='inroom_" + dataObj.list[i].fd + "'>" +
            ...
            let text = dataObj.list[i].name;

            var colors=["orangered","#bce8f1","lightgreen","lightpink", "lightskyblue", "lightblue", "lightcoral"];

            img = "<li>" +"<a style='text-decoration: none' href=\"javascript:selectUser('"
                + dataObj.list[i].fd + "')\">" + "<div style='font-size: 20px;background-color:" + colors[Math.floor(Math.random()*colors.length)] +
                ";width: 50px;height: 50px;color:white;text-decoration:none;" +
                "border-radius: 10px;text-align:center;line-height: 50px;'> " +  sub(text, 4)  + "</div></li></a>"
        }
